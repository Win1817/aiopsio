...
WEBUI_NAME = os.environ.get('WEBUI_NAME', 'ai.ops')
if WEBUI_NAME != 'ai.ops':
    WEBUI_NAME += ' (ai.ops)'
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('ai-ops')}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data('ai_ops', 'CHANGELOG.md') or b'').decode()
...
if FROM_INIT_PY:
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'ai_ops_data', 'zip', DATA_DIR)
...
REDIS_KEY_PREFIX = os.environ.get('REDIS_KEY_PREFIX', 'ai-ops')
...
OTEL_SERVICE_NAME = os.environ.get('OTEL_SERVICE_NAME', 'ai-ops')
