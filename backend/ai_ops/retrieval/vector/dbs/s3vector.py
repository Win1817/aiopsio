...
from ai_ops.retrieval.vector.utils import process_metadata
from ai_ops.retrieval.vector.main import (
    VectorDBBase,
    VectorItem,
    GetResult,
    SearchResult,
)
from ai_ops.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for ai.ops Knowledge.
    """
    ...
    def _filter_metadata(self, metadata: Dict[str, Any], item_id: str) -> Dict[str, Any]:
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual ai.ops metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The ai.ops GetResult expects lists of lists, so we wrap each list
            if all_ids:
