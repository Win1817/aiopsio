...
from ai_ops.utils.payload import (
...
async def send_post_request(
    ...
    try:
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f'ai.ops: Server Connection Error',
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else 'ai.ops: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r, session)
...
async def verify_connection(form_data: ConnectionVerificationForm, user=Depends(get_admin_user)):
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail='ai.ops: Server Connection Error')
        except Exception as e:
    ...
    if url_idx is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'ai.ops: Server Connection Error',
            )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = 'ai.ops: Server Connection Error'
            if r is not None:
                try:
                    res = r.json()
                    if 'error' in res:
