<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_ai_ops_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'ai.ops version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						OPEN_WEBUI_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_ai_ops_version ?? '0.0.0'
					}
				)
			);
			return;
